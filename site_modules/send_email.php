<?php
function sendEmail($email, $type, $additional_data = []) {
    $username = htmlspecialchars(($_SESSION['first_name'] ?? '') . ' ' . mb_substr($_SESSION['last_name'] ?? '', 0, 1, 'UTF-8') . '.');
    $year = date("Y");
    $styles = "
    <style type='text/css'>
        * {
    ...
    switch($type) {
        ...
        case 'suspicious_login':
            $ip = $additional_data['ip'] ?? 'неизвестный';
            $attempts = $additional_data['attempts'] ?? 'несколько';
            $subject = 'MCM | 🚨 Обнаружена подозрительная активность';
            $content = "
                <div class='warning'>
                    <h2>🚨 Обнаружена подозрительная активность</h2>
                </div>
                <p>Здравствуйте, <strong>{$username}</strong>!</p>
                <p>Наша система безопасности обнаружила необычную активность с вашим аккаунтом MCM.</p>
                </p>Было зафиксировано {$attempts} неудачных попыток входа с IP-адреса: <strong>{$ip}</strong>.</p>
                
                <div class='decoration'></div>
                
                <p><strong>Если это были вы:</strong></p>
                <p>• Убедитесь, что вы вводите правильный логин и пароль<br>
                   • Проверьте, не включен ли CAPS LOCK<br>
                   • Если вы забыли пароль, воспользуйтесь функцией восстановления</p>
                
                <p><strong>Если это были не вы:</strong></p>
                <p>• Рекомендуем удостовериться, что у вас стоит надёжный пароль<br>
        ...
        case 'forgot_password':
            ...
            $content = "
                ...
                <p>• Ссылка для сброса пароля действительна в течение 24 часов<br>
