<?php
    ...
    function calculateBonus($revenue) {
        if ($revenue > 1.0) {
            $excess = $revenue - 1.0;
            return $excess * 0.005; // 0.5% от превышения
        }
        return 0;
    }
    ...
    function formatMoney($amount) {
        if ($amount >= 1000000) {
            return number_format($amount / 1000000, 2, ',', ' ') . ' млн. ₽';
        } elseif ($amount >= 1000) {
            return number_format($amount / 1000, 1, ',', ' ') . ' тыс. ₽';
        } else {
...
<head>
    ...
    <title>5 | Отчет по выручке</title>
    <link rel="icon" type="image/svg+xml" href="https://logosandtypes.com/wp-content/uploads/2020/11/pyaterochka.svg">
    <style>
        * {
            margin: 0;
            padding: 0;
        ...
        body {
            ...
            background: linear-gradient(135deg, #e50000 0%, #4b0000 100%);
            min-height: 100vh;
            padding: 20px;
        }
...
<body>
    <div class="container">
        <div class="header">
            ...
            <div class="subtitle">Отчет по месячной выручке кассиров</div>
        </div>
        ...
        <div class="content">
            <div class="table-container">
                <table>
                    ...
                    <tbody>
                        <?php
                        foreach ($cashiers as $name => $revenue):
                            $bonus = calculateBonus($revenue);
                            $revenueInRubles = $revenue * 1000000; // МЛН в Руб
                            ?>
                            <tr>
                                <td><strong><?= htmlspecialchars($name) ?></strong></td>
                                <td><?= formatMoney($revenueInRubles) ?></td>
                                <td class="<?= $bonus > 0 ? 'bonus-positive' : 'bonus-zero' ?>">
                                    <?= formatMoney($bonus * 1000000) ?>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
            ...
            <div class="stats">
                <div class="stat-card">
                    ...
                    <div class="stat-value">
                        <?php
                        $totalRevenue = array_sum($cashiers) * 1000000;
                        echo formatMoney($totalRevenue);
                        ?>
                    </div>
                </div>
                ...
                <div class="stat-card">
                    ...
                    <div class="stat-value">
                        <?php
                        $totalBonus = 0;
                        foreach ($cashiers as $revenue) {
                            $totalBonus += calculateBonus($revenue);
                        }
                        echo formatMoney($totalBonus * 1000000);
                        ?>
                    </div>
                </div>
                
                <div class="stat-card">
                    <div class="stat-label">Кассиров с премией</div>
                    <div class="stat-value">
                        <?php
                        $withBonus = 0;
                        foreach ($cashiers as $revenue) {
                            if ($revenue > 1.0) $withBonus++;
                        }
                        echo $withBonus . ' из ' . count($cashiers);
                        ?>
                    </div>
                </div>
            </div>
        ...
        <div class="footer">
            © 2025 «Пятёрочка» / «X5 клуб». Все права защищены.
        </div>
    </div>
</body>
</html>